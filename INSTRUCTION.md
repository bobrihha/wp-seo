...
## 13) Дополнительная настройка (ОБЯЗАТЕЛЬНО)

### А) Плагин для SEO (Yoast)
Чтобы скрипт мог заполнять поля Yoast SEO (Title, Description), WordPress требует разрешения.
1. Зайдите в папку `wp_plugin` в этом проекте.
2. Там лежит файл `seo-rest-enable.php`.
3. Заархивируйте его в ZIP (или просто скопируйте) и установите/загрузите как плагин в WordPress (`Плагины -> Добавить новый -> Загрузить`).
4. Активируйте плагин "AI Content Hub - Yoast SEO REST Enabler".

**Без этого поля SEO останутся пустыми!**

### Б) Красивые стили (CSS)
Чтобы блоки "Важно", "Выводы" и таблицы выглядели красиво:
1. Откройте файл `css/ai-content-styles.css`.
2. Скопируйте всё содержимое.
3. В WordPress зайдите в `Внешний вид -> Настроить -> Дополнительные стили`.
4. Вставьте код туда и нажмите "Опубликовать".
