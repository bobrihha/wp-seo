...
- поддерживает **автопилот**: по расписанию проверяет источники, генерирует статьи и отправляет в WordPress (в черновики или сразу публикует).
...
### Автопилот (серверный режим)
Автопилот — отдельный скрипт:
- разовый запуск: `python3 autopilot.py --once`
- постоянная работа (по настройкам интервалов): `python3 autopilot.py --daemon`

Настройка автопилота в интерфейсе:
Sidebar → **Автопилот** → включить, выбрать источники, режимы (publish/draft/off), интервалы и лимиты.

### Автопилот через Docker Compose
В `docker-compose.yml` есть отдельный сервис `autopilot`, который работает постоянно, но ничего не делает пока в настройках выключен `Автопилот включён`.

Команды:
- запустить всё: `docker compose up -d --build`
- остановить только автопилот: `docker compose stop autopilot`
- запустить только автопилот: `docker compose up -d autopilot`
- логи автопилота: `docker compose logs -f autopilot`
...
В админке приложения → Sidebar → **Настройки** → вкладка **Ключи** → блок **WordPress**.
...
**Статус записи**
- **Черновик**: запись уйдёт в drafts (можно проверить/отредактировать перед публикацией).
- **Опубликовать сразу**: запись будет опубликована сразу после отправки.
...
Sidebar → **Настройки** → вкладка **Ключи** → блок **AI (OpenAI-compatible)**
...
- Дополнительные настройки (язык/стиль/формат/настроение/объём/заголовки) задаются в блоке **Контент (язык/стиль/объём)** и автоматически добавляются к промпту.
...
Sidebar → **Настройки** → вкладка **Изображения**
...
Если картинки выглядят слишком большими — отредактируйте в этом же CSS `img.ai-cover` и `img.ai-inline` (параметр `max-width`).
